...
1. A fixed seed, city weights, and member count deterministically generate the synthetic member source.
...
4. Durable member changes update the current member dimension, while immutable analytical snapshots preserve
   the exact state used by each run.
